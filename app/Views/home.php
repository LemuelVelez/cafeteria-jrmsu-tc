    ...
    <div class="container py-4">
        ...
        <div class="row g-4">
            <?php if ($products): foreach ($products as $product): ?>
                <div class="col-sm-6 col-lg-4"><article class="product-card bg-white"><?php if ($product['image']): ?><img class="product-image" src="<?= base_url($product['image']) ?>" alt="<?= esc($product['name']) ?>"><?php else: ?><div class="product-placeholder"><img src="<?= base_url('assets/img/jrmsu-cafeteria-logo.png') ?>" alt=""></div><?php endif; ?><div class="p-4"><div class="small text-secondary mb-2"><?= esc($product['category_name']) ?></div><div class="d-flex justify-content-between gap-3"><h3 class="h5 section-title fw-bold"><?= esc($product['name']) ?></h3><span class="price text-nowrap"><?= format_price($product['price']) ?></span></div><p class="text-secondary small mb-0"><?= esc($product['description']) ?></p></div></article></div>
            <?php endforeach; else: ?>
