...
class LoginController extends BaseController
{
    ...
    public function store()
    {
        ...
        if (! service('throttler')->check($key, 5, MINUTE)) {
            return redirect()->back()->withInput()->with('error', 'Too many sign-in attempts. Please try again in one minute.');
        }
        ...
        if (! (new AuthService())->attempt((string) $this->request->getPost('email'), (string) $this->request->getPost('password'))) {
            return redirect()->back()->withInput()->with('error', 'Invalid email, password, or account status.');
        }
